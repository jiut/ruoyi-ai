...
public class UserRegistrationController extends BaseController {
    ...
    @Operation(
        summary = "获取用户档案完整度",
        description = "计算用户各身份的档案完整度并提供改进建议",
        responses = {
            @ApiResponse(responseCode = "200", description = "获取成功",
                        content = @Content(schema = @Schema(implementation = ProfileCompletenessVo.class))),
            @ApiResponse(responseCode = "401", description = "用户未登录")
        }
    )
    ...
    @Operation(
        summary = "添加教育背景",
        description = "为当前用户绑定的设计师添加教育背景信息",
        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
            description = "教育背景信息",
            required = true,
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = Education.class),
                examples = @ExampleObject(
                    name = "教育背景示例",
                    value = """
                        {
                            ...
                            "endDate": "2021-06-30",
                            "isCurrent": false,
                            "description": "主修课程：视觉设计、品牌设计、数字媒体设计等",
                            "gpa": 3.8,
                            "ranking": 5,
                            "totalStudents": 120
                        }
    ...
    @Log(title = "添加教育背景", businessType = BusinessType.INSERT)
    @PostMapping("/education")
    public R<Void> addEducation(@Validated @RequestBody Education education) {
        Long userId = LoginHelper.getUserId();
        
        // 获取设计师身份
        UserBinding binding = userBindingService.getBindingByUserIdAndEntityType(userId, UserEntityType.DESIGNER);
        if (binding == null) {
            return R.fail("用户未绑定设计师身份");
        }
        
        // 设置设计师ID
        education.setDesignerId(binding.getEntityId());
        
        // 保存教育背景
        if (educationService.insertEducation(education)) {
            return R.ok();
        }
    ...
    @Operation(
        summary = "添加工作经历",
        description = "为当前用户绑定的设计师添加工作经历信息",
        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
            description = "工作经历信息",
            required = true,
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = WorkExperience.class),
                examples = @ExampleObject(
                    name = "工作经历示例",
                    value = """
                        {
                            ...
                            "endDate": "2024-03-30",
                            "isCurrent": false,
                            "description": "负责微信团队的UI设计工作，参与多个重要功能的设计",
                            "location": "深圳",
                            "industry": "互联网"
                        }
                        """
    ...
    public R<Void> addWorkExperience(@Validated @RequestBody WorkExperience workExperience) {
        ...
        // 设置设计师ID
        workExperience.setDesignerId(binding.getEntityId());
        
        // 保存工作经历
        if (workExperienceService.insertWorkExperience(workExperience)) {
            return R.ok();
        }
    ...
    @Operation(
        summary = "上传作品集",
        description = "为当前用户绑定的设计师上传作品信息",
        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
            description = "作品信息",
            required = true,
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = Work.class),
                examples = @ExampleObject(
                    name = "作品示例",
                    value = """
                        {
                            ...
                            "workType": "image",
                            "fileUrl": "https://example.com/work1.jpg",
                            "thumbnailUrl": "https://example.com/work1_thumb.jpg",
                            "tags": "[\\"UI设计\\", \\"移动应用\\", \\"简约风格\\"]",
                            "isFeatured": true
                        }
                        """
    ...
    @Log(title = "上传作品集", businessType = BusinessType.INSERT)
    @PostMapping("/works")
    public R<Void> addWork(@Validated @RequestBody Work work) {
        ...
        // 设置设计师ID
        work.setDesignerId(binding.getEntityId());
        
        // 保存作品
        if (workService.insertWork(work)) {
            return R.ok();
        }
